...
当前部署在 Ubuntu 22.04 云服务器。宿主机已有 OMS 服务占用公网 `80` 端口，因此保留宿主机 Nginx，并由其按公网 IP 将请求反向代理至仅监听 `127.0.0.1:8080` 的 CampusHub Nginx 容器。数据库、Redis 和后端均不暴露宿主机端口。已在 2026-08-05 验证后端健康检查、容器前端入口和公网入口均返回成功状态。
