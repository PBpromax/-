...
## 2026-08-05 - Task 9 - 公网部署与运行验证

- 部署目标：阿里云 Ubuntu 22.04 云服务器，公网地址 `http://101.132.133.42`。
...
## 2026-08-07 - Task 11 - 线上数据库连接与初始化修复

- 用户反馈：注册页面显示“服务端异常”。
- 日志排查：后端容器的 MySQL 连接被拒绝，随后确认当前 MySQL 数据卷未包含 `campushub_db`。根因是服务器上的环境变量凭据与保留数据卷中的数据库账户状态不一致，且首次初始化未完成；前端、CORS 和注册请求格式均正常。
- 修复：在不删除 Docker 卷的前提下，通过 MySQL 维护模式恢复根账户访问，为应用账户授予 Docker 内网（`%`）访问 `campushub_db` 的权限，并用仓库内 `mysql-init/init.sql` 初始化数据库、表结构和演示数据。
- 验证：通过 Nginx 入口提交一个新的注册请求，返回 `{"code":200,"message":"注册成功"}`；该新账号登录返回 HTTP `200`。`OPTIONS /api/v1/auth/register` 返回 HTTP `200`，且 `Access-Control-Allow-Origin` 为 `http://101.132.133.42:8088`；从公网请求前端入口亦返回 HTTP `200`。
- 安全处理：排障使用临时 SSH 密钥，完成验证后从服务器授权列表和本机删除；数据库/JWT 凭据未写入仓库或本日志。
